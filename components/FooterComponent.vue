    ...
    <div class="section footer--inner flex flex-wrap">
      <div class="footer--column flex-2 m-4" style="flex-basis: 15rem;">
        ...
        <p class="mb-2">
          ...
          being built by
          <nuxt-link :to="{ path: '/kalenjordan' }">
            Kalen
          </nuxt-link>
          with &hearts; in
          <nuxt-link :to="{ path: '/s/austin' }">
            Austin
          </nuxt-link>
          .
        </p>
      ...
      <div class="footer--column flex-1 m-4" style="flex-basis: 10rem;">
        ...
        <ul class="list-reset">
          <template v-for="savedSearch in savedSearches">
            <li v-if="savedSearch.icon !== 'location_on'" :key="savedSearch.id" class="pb-2">
              <nuxt-link class="naked-link" :to="{ path: '/s/' + savedSearch.slug }">
                {{ savedSearch.name }}
              </nuxt-link>
            </li>
          </template>
      ...
      <div class="footer--column flex-1 m-4" style="flex-basis: 15rem;">
        ...
        <div class="mb-4">
          <ul class="list-reset">
            <li class="pb-2">
              <nuxt-link class="naked-link" :to="{ path: '/search'}">
                Search
              </nuxt-link>
            </li>
            <li class="pb-2">
        ...
        <div v-if="loggedInUser && loggedInUser.is_admin">
          <div class="mb-2">
            Admin Tasks
          </div>
